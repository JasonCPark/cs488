...
int gr_sphere_cmd(lua_State* L)
{
  ...
  if (lua_gettop(L) > 2) {
    get_tuple(L, 2, &motionTransl[0], 3);
  }

  data->node = new GeometryNode( name, new Sphere(), nullptr, motionTransl );
...
int gr_cube_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new MeshPrim(mesh));
...
int gr_nh_sphere_cmd(lua_State* L)
{
  ...
  vec3 motionTransl = vec3(0,0,0);
  if (lua_gettop(L) > 4) {
    get_tuple(L, 4, &motionTransl[0], 3);
  }

  GeometryNode* geoNode = new GeometryNode(name, new Sphere(), nullptr, motionTransl);
...
int gr_nh_box_cmd(lua_State* L)
{
  ...
  GeometryNode* geoNode = new GeometryNode(name, new MeshPrim(mesh));
  geoNode->scale(vec3(size,size,size));
  geoNode->translate(pos);
  data->node = geoNode;
